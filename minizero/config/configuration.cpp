...
int zero_display_latest_games = 0;
bool zero_use_population = false;
int zero_population_size = 2;
int zero_population_snapshot_interval = 50;
int zero_population_rotation_interval = 10;
float zero_population_hard_ratio = 0.7f;
...
bool zero_use_jpsro = false;
std::string zero_jpsro_profile_file = "";
bool zero_jpsro_eval_only = false;
...
void setConfiguration(ConfigureLoader& cl)
{
    ...
    cl.addParameter("zero_display_latest_games", zero_display_latest_games, "the number of latest games to display statistics in log; 0 to disable", "Zero");
    cl.addParameter("zero_use_population", zero_use_population, "true for training against historical multiplayer lineups", "Zero");
    cl.addParameter("zero_population_size", zero_population_size, "the number of historical checkpoints retained by the population server", "Zero");
    cl.addParameter("zero_population_snapshot_interval", zero_population_snapshot_interval, "minimum number of training iterations between historical population checkpoints", "Zero");
    cl.addParameter("zero_population_rotation_interval", zero_population_rotation_interval, "number of training iterations before rotating each worker's historical opponent", "Zero");
    cl.addParameter("zero_population_hard_ratio", zero_population_hard_ratio, "mixture weight of hard-lineup sampling versus uniform sampling", "Zero");
    ...
    cl.addParameter("zero_use_jpsro", zero_use_jpsro, "true for training current seats against weighted frozen joint profiles", "Zero");
    cl.addParameter("zero_jpsro_profile_file", zero_jpsro_profile_file, "TSV JPSRO or guided-pool profile plan generated by tools/jpsro.py", "Zero");
    cl.addParameter("zero_jpsro_eval_only", zero_jpsro_eval_only, "allow all-frozen JPSRO profiles for batched payoff evaluation", "Zero");
