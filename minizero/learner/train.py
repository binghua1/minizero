...
class MinizeroDadaLoader:
    def __init__(self, conf_file_name):
        ...
        if py.get_nn_type_name() == "alphazero":
            ...
            self.progress = np.zeros(py.get_batch_size() * self.value_output_size, dtype=np.float32)
            self.reward = None
        else:
            ...
            self.progress = np.zeros(py.get_batch_size() * py.get_nn_discrete_value_size(), dtype=np.float32)
    ...
    def sample_data(self, device='cpu'):
        self.data_loader.sample_data(self.features, self.action_features, self.policy, self.value, self.progress, self.reward, self.loss_scale, self.sampled_index)
        ...
        if py.get_nn_type_name() == "alphazero":
            ...
            progress = torch.FloatTensor(self.progress).view(py.get_batch_size(), 1, self.value_output_size).to(device)
        else:
        ...
        return features, action_features, policy, value, progress, reward, loss_scale, sampled_index
...
def calculate_progress_loss(network_output, label_progress, loss_scale):
    if "progress" not in network_output or py.get_progress_loss_scale() <= 0:
        return None
    progress_error = nn.functional.mse_loss(network_output["progress"], label_progress, reduction='none')
    return (progress_error.view(progress_error.shape[0], -1).mean(dim=1) * loss_scale).mean()
...
def train(model, training_dir, data_loader, start_iter, end_iter):
    ...
    for i in range(1, py.get_training_step() + 1):
        ...
        features, action_features, label_policy, label_value, label_progress, label_reward, loss_scale, sampled_index = data_loader.sample_data(model.device)
        ...
        if py.get_nn_type_name() == "alphazero":
            ...
            loss_progress = calculate_progress_loss(network_output, label_progress[:, 0], loss_scale)
            if loss_progress is not None:
                loss += py.get_progress_loss_scale() * loss_progress
            ...
            if loss_progress is not None:
                add_training_info(training_info, 'loss_progress', loss_progress.item())
