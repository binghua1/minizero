...
usage() {
    ...
    train)
        ...
        echo "             --continue-training    Resume an existing training directory without prompts"
        ;;
    self-eval)
...
while [[ $1 ]]; do
    ...
    --continue-training) continue_training=true; ;;
...
if [[ $mode == train ]]; then # ================================ TRAIN ================================
    ...
    # launch zero server
    {
        $launch scripts/zero-server.sh $game $conf_file $zero_end_iteration -n "$train_dir" -g ${CUDA_VISIBLE_DEVICES//,/} \
            ${continue_training:+--continue} -conf_str "$conf_str"
    ...
    if [[ $game == tictacmo || $game == connect3x3 || $game == go3 || $game == blokus || $game == blokus10 || $game == blokus15 ]]; then
